  ...
  return (
    <div id="request-list-wrap">
      <table id="request-table">
        <thead>
          <tr><th className="col-status">Status</th><th className="col-method">Method</th><th className="col-type">Type</th><th className="col-url">URL</th><th className="col-security">Security</th><th className="col-match">Replay</th><th className="col-size">Size</th><th className="col-time">Time</th></tr>
        </thead>
        <tbody>
          {requests.map((req) => {
            const url = parseUrlParts(req.url);
            const security = analyzeRequestSecurity(req);
            return (
              <tr key={req.id} className={`${selectedId === req.id ? 'selected ' : ''}security-row-${security.rating}`} onClick={() => onSelect(req.id)}>
                ...
                <td><span className={`security-pill security-${security.rating}`} title={`${security.findings.length} observed security finding${security.findings.length === 1 ? '' : 's'}`}>{security.score == null ? security.label : `${security.label} ${security.score}`}</span></td>
